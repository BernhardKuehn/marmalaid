...
#' Raster files from both the 'raster'-package (\code{RasterBrick}) and the 'terra' (\code{SpatRaster}) will work here.
...
spatial.PCA = function(x,center = TRUE , scale. = FALSE, spatial.extent = "North Sea",plot = FALSE,var.threshold = 0.95)  {
  ...
  if(class(x)=="list") {
    if(all(lapply(x,class)%in%c("raster","RasterBrick","RasterStack"))
      ...
      if(is.character(spatial.extent) == TRUE) {
        if(spatial.extent == "North Sea") {
          ...
          if(use.terra == TRUE){
            spatial.extent = terra::ext(c(xmin = -7.01,
                                          xmax = 12.9,
                                          ymin = 48.9,
                                          ymax = 61))
          } else {
      ...
      for(i in 1:length(x)) {
        if(use.terra == TRUE){
          x.crop[[i]] = terra::crop(x[[i]],y = spatial.extent)
        } else {
      ...
      for(i in 1:length(geo.pos)) {
        if(use.terra == TRUE){
          geo.pos[[i]] = terra::xyFromCell(x.crop[[i]],1:terra::ncell(x.crop[[i]]))
        } else {
      ...
      for(i in 1:length(x)) {
        if(use.terra == TRUE){
          tmp.mat[[i]] = t(terra::as.matrix(x.crop[[i]]))
        } else {
    ...
    # check if terra package should be used or not
  ...
  # crop to spatial extent
  if(use.terra == TRUE){
    x.crop = terra::crop(x,y = spatial.extent)
    # convert to matrix
    print("1. convert raster to matrix...")
    mat = t(terra::as.matrix(x.crop))
    # save geo.pos of raster grid cells
    geo.pos = terra::xyFromCell(x.crop,1:terra::ncell(x.crop))
  } else {
  ...
  sp.PCA.df = data.frame(geo.pos,sp.PCA$rotation[,1:indx.PCs,drop = F])
  # rasterize
  if(class(x)=="list") {
    ...
    for(i in 1:length(x)) {
      ...
      if(use.terra == TRUE){
        sp.PCA.raster = terra::rast(sp.PCA.df,type = "xyz")
        terra::crs(sp.PCA.raster) = terra::crs(x)
      } else{
        sp.PCA.raster = raster::rasterFromXYZ(sp.PCA.df)
        # define coordinate reference system
        raster::crs(sp.PCA.raster) = raster::crs(x)
      }
  ...
  if(plot == TRUE) {
    ...
    for(i in 1:indx.PCs) {
      if(use.terra == TRUE){
        terra::image(sp.PCA.raster[[i]], col = grDevices::colorRampPalette(RColorBrewer::brewer.pal(10,"RdYlBu"))(100),las = 1)
        terra::contour(x = sp.PCA.raster[[i]], add=TRUE, col="gray30")
  ...
  if(use.terra == TRUE){
    year = as.numeric(substr(names(x),start = 2,stop = 5))
    month = as.numeric(substr(names(x),start = 7,stop = 8))
    day = as.numeric(substr(names(x),start = 10,stop = 11))
  } else {
